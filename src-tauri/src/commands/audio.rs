    ...
    F: FnOnce(&Mixer, &Selem) -> Result<T, String>,
{
    ...
    let selem = mixer.find_selem(&selem_id).ok_or("No se encontró el control Master")?;
    f(&mixer, &selem)
}
...
pub fn get_volume() -> Result<VolumeInfo, String> {
    with_mixer(|_mixer, selem| {
        ...
        let is_muted = selem.get_playback_switch(alsa::mixer::SelemChannelId::mono())
            .map_err(|e| e.to_string())?
            == 0;
...
pub fn toggle_mute() -> Result<bool, String> {
    with_mixer(|_mixer, selem| {
        let current = selem.get_playback_switch(alsa::mixer::SelemChannelId::mono())
            .map_err(|e| e.to_string())?;
        
        let new_state = if current == 1 { 0 } else { 1 };
        selem.set_playback_switch_all(new_state)
            .map_err(|e| e.to_string())?;
