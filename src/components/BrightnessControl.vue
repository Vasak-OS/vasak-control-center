<template>
  <div class="bg-white/50 dark:bg-black/50 rounded-xl flex flex-row items-center gap-2 justify-between w-full h-auto p-4 transition-all duration-200 hover:bg-white/60 dark:hover:bg-black/60">
    <button class="w-8 h-8 flex items-center justify-center rounded-lg transition-all duration-200 hover:bg-white/30 dark:hover:bg-black/30 hover:scale-110 active:scale-95">
      <img 
        :src="currentIcon" 
    ...
    />
    <span class="w-12 text-right transition-all duration-200 font-medium" :class="{ 'text-yellow-500': brightnessPercentage > 80, 'text-orange-500': brightnessPercentage < 20 }">{{ brightnessPercentage }}%</span>
  </div>
</template>
...
input[type="range"]:hover::-webkit-slider-thumb {
  ...
  box-shadow: 0 4px 16px rgba(234, 179, 8, 0.4), 0 0 0 4px rgba(234, 179, 8, 0.1);
  background: #ca8a04;
}

input[type="range"]:active::-webkit-slider-thumb {
  transform: scale(1.35);
  box-shadow: 0 2px 12px rgba(234, 179, 8, 0.6), 0 0 0 6px rgba(234, 179, 8, 0.2);
  background: #a16207;
}
